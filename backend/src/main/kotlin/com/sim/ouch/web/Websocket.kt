    ...
    Consumer<WsHandler> {
        val connect = ConnectHandler { session ->
            launch {
                ...
                if (name.isNullOrBlank()) return@launch session.close(
                    ER_NO_NAME)
        ...
        val message = MessageHandler { session, msg ->
            launch {
                ...
                when (packet.dataType) {
                    ...
                    EXISTENCE -> TODO()
                    ACTION -> TODO()
                    CHAT -> ex?.chat?.update(session.quidity()!!,
                        packet.data as String)?.let {
                        ex.sessions?.broadcast(CHAT, it)
                    }
        ...
        val close = CloseHandler { session, statusCode, reason ->
            launch {
                ...
                session.quidity()?.also {
                    session.existence()?.sessions?.broadcast(EXIT, it,
                        session.id)
                }
